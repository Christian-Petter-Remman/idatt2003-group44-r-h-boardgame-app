 ...
 *
 * JavaFX view class that renders the memory game board, handles user interaction,
 * displays scores, turn information, and responds to updates in the game model.
 */
public class MemoryGameView implements MemoryGameObserver {
  ...
  public void initialize(MemoryGameSettings settings) {
    ...
    root.getStylesheets().add(Objects.requireNonNull(getClass().getResource("/css/MemoryGameStyleSheet.css")).toExternalForm());
  ...
  private void updatePairIcons(HBox box, MemoryBoardGame board, Set<String> ids) {
    ...
    ids.stream()
            ...
            .forEach(card -> {
              ImageView iv = new ImageView(new Image(Objects.requireNonNull(getClass().getResourceAsStream(card.getImagePath()))));
              iv.setFitWidth(24);
              iv.setFitHeight(24);
  ...
  private void setupGrid(MemoryGameSettings settings) {
    ...
    for (int i = 0; i < rows * cols; i++) {
      ImageView iv = new ImageView(new Image(Objects.requireNonNull(getClass().getResourceAsStream("/images/card_back.png"))));
  ...
  private void setupSidebar(MemoryGameSettings settings) {
    ...
    VBox p1 = new VBox(4, new Label(settings.getPlayers().getFirst().getName()), player1Score, player1Pairs);
    VBox p2 = new VBox(4, new Label(settings.getPlayers().get(1).getName()), player2Score, player2Pairs);
